...
<img src="./image-1.png" alt="image-1" style="zoom:33%;" />
...
<img src="./image-2.png" alt="image-2" style="zoom: 33%;" />
...
single-spa首先在主应用中注册所有子应用的路由，当URL改变时就会去进行匹配，匹配到哪个子应用就会去加载对应的那个子应用。

相对于iframe的实现方案，single-spa中主应用和各个子应用之间共享着一个全局上下文，并且不存在URL不同步和UI不同步的情况，但是single-spa也有以下的缺点：

1. 没有实现js隔离和css隔离
2. 需要修改大量的配置，包括主应用和子应用的，不能开箱即用
...
- 主应用：主要负责集成所有的子应用，提供一个入口能够访问你所需要的子应用的展示，尽量不写复杂的业务逻辑
- 子应用：根据不同业务划分的模块，每个子应用都打包成`umd`模块的形式供主应用（主应用）来加载

#### 主应用

主应用 用的是`create-react-app`脚手架加上`antd`组件库搭建的项目，也可以选择vue或者其他框架，一般来说，主应用只提供加载子应用的容器，尽量不写复杂的业务逻辑。
...
至此主应用就改造完成，如果是老项目或者其他框架的项目想改成微前端的方式也是类似。
...
defineConfig({
    base: '/sub-vue', // 和主应用中配置的activeRule一致
    server: {
      port: 3002,
      cors: true,
...
qiankun实现了各个子应用之间的样式隔离，但是主应用和子应用之间的样式隔离没有实现，所以主应用和子应用之前的样式还会有冲突和覆盖的情况。
...
具体方案：在主应用中复写并监听`history.pushState()`方法并做相应的跳转逻辑
...
qinkun提供了一个全局的`GlobalState`来共享数据，主应用初始化之后，子应用可以监听到这个数据的变化，也能提交这个数据。
